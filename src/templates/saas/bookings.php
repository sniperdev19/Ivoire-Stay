...
<div x-data="{
  ...
  form: { room_id:'', check_in:'', check_out:'', client_name:'', client_email:'', client_phone:'', booking_type:'nuit', source:'manual', notes:'' },
  ...
  async loadBookings() {
    ...
    this.error = null;
    try {
      let url = this.apiUrl('/api/bookings')
      ...
      if (data.success) {
        ...
        this.bookings = raw.map(b => ({
          ...
          room_name: b.room_name ?? b.room?.name ?? 'Chambre #' + b.room_id,
          total_price: b.total_price ?? b.total_amount ?? 0,
          nights: b.nights ?? (b.check_in && b.check_out ? Math.round((new Date(b.check_out) - new Date(b.check_in)) / 86400000) : 1),
        }));
        this.total = data.data?.total ?? this.bookings.length;
        // Ne PAS activer le fallback si l'API répond avec succès
      } else {
        // Seulement si l'API échoue vraiment
  ...
  async applyFilters() { this.page = 1; await this.loadBookings(); },
  async resetFilters() { this.filterStatus='all'; this.filterSearch=''; this.filterDateFrom=''; this.filterDateTo=''; await this.applyFilters(); },
  ...
  async openDetail(booking) {
    ...
    try {
      const res = await fetch(this.apiUrl('/api/bookings/' + booking.id), { headers: this.apiHeaders() });
      const data = await res.json();
  ...
  async updateStatus(bookingId, newStatus) {
    ...
    try {
      let endpoint = this.apiUrl('/api/bookings/' + bookingId);
      let method = 'PUT';
      let body = JSON.stringify({ status: newStatus });
      if (newStatus === 'checked_in') { endpoint = this.apiUrl('/api/bookings/' + bookingId + '/checkin'); method='POST'; body = null; }
      if (newStatus === 'checked_out') { endpoint = this.apiUrl('/api/bookings/' + bookingId + '/checkout'); method='POST'; body = null; }
  ...
  async deleteBooking(id) {
    ...
    try {
      const res = await fetch(this.apiUrl('/api/bookings/' + id), { method: 'DELETE', headers: this.apiHeaders() });
      const data = await res.json();
      if (data.success) {
        this.bookings = this.bookings.filter(b => b.id !== id);
        this.showDetail = false; this.selectedBooking = null;
  ...
  async createBooking() {
    this.createError = null; this.createLoading = true;
    try {
      const res = await fetch(this.apiUrl('/api/bookings'), { method:'POST', headers:this.apiHeaders(), body: JSON.stringify({ ...this.form, establishment_id:this.estId() }) });
      const data = await res.json();
      if (data.success) {
        const created = data.data?.booking ?? data.data; if (created) this.bookings.unshift(created);
        this.showCreate = false; this.resetForm(); this.showToast('Réservation créée avec succès.', 'success');
      } else {
        this.createError = data.message ?? 'Erreur lors de la création.'; if (data.errors) this.createError += ' ' + JSON.stringify(data.errors);
      }
    } catch (e) { this.createError = 'Erreur réseau. Veuillez réessayer.'; }
    finally { this.createLoading = false; }
  },

  resetForm() { this.form = { room_id:'', check_in:'', check_out:'', client_name:'', client_email:'', client_phone:'', booking_type:'nuit', source:'manual', notes:'' }; this.createError = null; },

  get totalPages() { return Math.max(1, Math.ceil(this.total / this.perPage)); },
  async goToPage(p) { if (p < 1 || p > this.totalPages) return; this.page = p; await this.loadBookings(); },

  get filteredBookings() {
    let list = [...this.bookings];
    if (this.filterStatus !== 'all') list = list.filter(b => b.status === this.filterStatus);
    if (this.filterSearch) { const q = this.filterSearch.toLowerCase(); list = list.filter(b => (b.client_name||'').toLowerCase().includes(q) || (b.room_name||'').toLowerCase().includes(q) || (b.client_phone||'').includes(q)); }
  ...
  countByStatus(s) { if (s === 'all') return this.bookings.length; return this.bookings.filter(b => b.status === s).length; },
  ...
  statusConfig(s) { return { confirmed:{ label:'Confirmée', badge:'badge badge-success' }, pending:{ label:'En attente', badge:'badge badge-warning' }, checked_in:{ label:'Arrivée', badge:'badge badge-info' }, checked_out:{ label:'Départ', badge:'badge badge-gold' }, cancelled:{ label:'Annulée', badge:'badge badge-danger' } }[s] ?? { label:s, badge:'badge' }; },
  sourceLabel(s) { return { manual:'Manuel', online:'En ligne', phone:'Téléphone', walk_in:'Sur place' }[s] ?? s; },
  ...
  nextActions(status) { const map = { pending:[{ label:'Confirmer', value:'confirmed', color:'success' },{ label:'Annuler', value:'cancelled', color:'danger' }], confirmed:[{ label:'Check-in', value:'checked_in', color:'info' },{ label:'Annuler', value:'cancelled', color:'danger' }], checked_in:[{ label:'Check-out', value:'checked_out', color:'gold' }], checked_out:[], cancelled:[] }; return map[status] ?? []; }
}"
 x-init="init()"
